...
{     
     ...
     {      
             response.querySelector('.listening')?.remove();
     ...
     else if(message.responseType == 'assistant' && message.text == '<FLUX-1>')
     {   
         e[e.length-1].innerHTML = `<div></div><div class = 'process'><div><i class="fa-solid fa-spinner fa-spin"></i></div></div><div class="revised-prompt"></div>`;
     ...
     {  
         ...
         e[e.length-1].querySelector('.process').innerHTML = `<img src="${message.image_url}" alt="Not found">`;
     ...
     {      
        ...
        if(message.text == "<stream>"){
            stream = true;
            return;
        }
        else if(message.text == "</stream>"){
            after_ai_prompt_generated();  
            stream = false;
            return;
        }
        if (stream == true)
        {  
            e[e.length-1].querySelector('div').innerText =   e[e.length-1].querySelector('div').innerText + `${message.text}`;
        }
      
        
     }
     else if(message.Type == 'Transcribing'){
           response.innerHTML = response.innerHTML + `<div class="listening"><div>...</div></div>`;
     }
