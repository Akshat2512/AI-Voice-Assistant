...
from backend.openai_models import transcribe_audio, generate_response, ChatHistory
    ...
    while True:
        ...
        try:
            ...
            if not response_queue.empty() or result:
              ...
              await generate_ai_response(response_queue, websocket, user_id, chat_history, prompt = result)   #  generates ai responses and send it back to the client
...
async def generate_ai_response(response_queue, websocket, user_id, chat_history, prompt):        
            ...
            #       os.makedirs(audio_path)
               ...
               result = convert_audio_bytes_to_wav_format(audio_data)
               if result == 'short_length':
                  return None
               else:
                  print("Transcribing ...")
                  await websocket.send_json({"Type": "Transcribing"})
                  await asyncio.sleep(0.1)
                  prompt = await transcribe_audio(websocket, result)  # generate texts from audio using whisper-1 model
            ...
            if len(prompt) >= 2:
                ...
                index = len(chat_history.process_lists)
                if index > 0 and chat_history.process_lists[index-1] == True:
                    chat_history.process_lists[index-1] = False
                await asyncio.sleep(0.1)

                def ai_response():
                    loop = asyncio.new_event_loop()  # Create a new event loop for the thread
                    asyncio.set_event_loop(loop)
                    loop.run_until_complete(generate_response(prompt, chat_history, index, websocket))
                    loop.close()
                    # asyncio.create_task(generate_response(prompt, os.getenv('OPENAI_API_KEY'), chat_history, index, websocket))

                response = threading.Thread(target = ai_response)  # generate natural language using gpt-4o model  
                response.start()
                await asyncio.sleep(0.1)
...
def convert_audio_bytes_to_wav_format(audio_data):    # save audio to the folder temporary.
       CHANNELS=1
       sample_width = 2
       RATE = 16000
    ...
    #    print(original_duration)
       if(original_duration > min_duration):
        ...
        #     file = open(f"backend/temp/recording_akshat.wav", 'wb')
        #     file.write(audio_stream.getvalue()) # save file to folder
        #     file.close()
         audio_stream.seek(0)
         return audio_stream
